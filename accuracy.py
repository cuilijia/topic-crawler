...
# 读取数据
def loadOriginData(src='file/train'):
    # train.data
...
def featureSel(doc2term, term2doc, cate2docs):
    ...
    for category in categories:
        ...
        for term in term2doc:
            ...
            if(((AVal + CVal) * (BVal + DVal) * (AVal + BVal) * (CVal + DVal))!=0):
             CHIVal = N * (AVal * DVal - CVal * BVal)**2 / ((AVal + CVal) * (BVal + DVal) * (AVal + BVal) * (CVal + DVal))
            else: CHIVal = N * (AVal * DVal - CVal * BVal)**2 / 1
...
if __name__ == '__main__':
    ...
    #vocSrc = r'file/testo1.txt'
    #voc = pd.read_table(vocSrc, names=['voc'])

    #for j in features:
    #  for i in range(0,voc.__len__()):
    #    if(j==i):
    #     print '%d  %s' %(i, voc['voc'][i])

    # build SVM model
    x, y = buildSVMData(TF, DF, features, len(doc2term), label, cate2docs, doc2term)
    # 读取测试数据(test.data)
    TF_test, doc2term_test, term2doc_test, cate2docs_test, label_test = loadOriginData(src='file/test')
    ...
    for doc_index, doc_label in enumerate(y_test):
        n+=1
        print ("test【%d】:第%s类（应为第%s类）") %(n,int(p_label[doc_index]),doc_label)
    ...
    for categoryErr in categoriesErrs:
        ...
        print('第%d类共 %d样本, 被错分的个数为 %d .\n'
              % (categoryErr, len(cate2docs_test[categoryErr]), numOfErr))
