...
def analysis():
  ...
  #  voc = pd.read_table(vocSrc, names=['voc'])
    ...
    # 读取测试数据(test.data)
    TF_test, doc2term_test, term2doc_test, cate2docs_test, label_test = loadOriginData(src='file/urlanalyse')
    ...
    for doc_index, doc_label in enumerate(y_test):
        n+=1
        RR=int(p_label[doc_index])
        print ("result:第%s类") %(int(p_label[doc_index]))
